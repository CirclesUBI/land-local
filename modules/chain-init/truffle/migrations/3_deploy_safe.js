...
const CompatibilityFallbackHandler = truffleContract(
...
module.exports = async function (deployer, network, accounts) {
  ...
  const proxyFactoryContract = await deployer
    .deploy(ProxyFactory, { from: accounts[0] })
    .then((result) => result.address.toLowerCase());

  addressCollection.proxyFactoryContract = proxyFactoryContract;
  console.log("Safe factory address is:", proxyFactoryContract);

  const compatibilityFallbackHandlerContract = await deployer
    .deploy(CompatibilityFallbackHandler, { from: accounts[0] })
    .then((result) => result.address.toLowerCase());

  addressCollection.compatibilityFallbackHandlerContract =
    compatibilityFallbackHandlerContract;
  console.log(
    "CompatibilityFallbackHandler address is:",
    compatibilityFallbackHandlerContract
  );

  const multiSendContract = await deployer
    .deploy(MultiSend, { from: accounts[0] })
    .then((result) => result.address.toLowerCase());

  addressCollection.multiSendContract = multiSendContract;
  console.log("MultiSend address is:", multiSendContract);

  const multiSendCallOnlyContract = await deployer
    .deploy(MultiSendCallOnly, { from: accounts[0] })
    .then((result) => result.address.toLowerCase());
  ...
  const safeProxyContract = await deployer
    .deploy(SafeProxy, addressCollection.masterSafeContract, {
      from: accounts[0],
    })
    .then((result) => result.address.toLowerCase());
